...
void InitializeMaterialData(SamplerState samplerMainTex,float2 uv, MaterialInputData i, out MaterialData m)
{
    // Albedo + Alpha
    m.albedoAlpha = SAMPLE_TEXTURE2D(i.baseMap, samplerMainTex, uv).rgba * i.baseColor;
    
    
    // Normals
    // To do: Could simplify this, probably
    float4 normalMap = SAMPLE_TEXTURE2D(i.normalMap, samplerMainTex, uv);
    ...
    {
        roughness = saturate(SAMPLE_TEXTURE2D(i.roughnessMap, samplerMainTex, uv).r + i.roughnessMapExposure);
    }
    ...
    {
        m.metalness = saturate(SAMPLE_TEXTURE2D(i.metalnessMap, samplerMainTex, uv).r + i.metalnessMapExposure);
    }
    ...
    // Occlusion
    m.occlusion = SAMPLE_TEXTURE2D(i.occlusionMap, samplerMainTex, uv).r;
